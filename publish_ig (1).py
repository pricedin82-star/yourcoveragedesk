...
def publicar_reel(video_url, caption, portada_url=None):
    """Los Reels tardan mucho más en procesarse que las fotos: Instagram tiene
    que transcodificar el video entero, así que el sondeo es más largo."""
    ig, token = env("IG_USER_ID"), env("META_ACCESS_TOKEN")
    payload = {
        "media_type": "REELS",
        "video_url": video_url,
    ...
    if portada_url:
        payload["cover_url"] = portada_url

    r = http(f"{BASE}/{ig}/media", payload)
    contenedor = r["id"]
    print(f"  contenedor {contenedor}; transcodificando")
    esperar_listo(contenedor, token, "el Reel", intentos=90, espera=10)
...
def main():
    ...
    p.add_argument("--reel", help="URL pública del mp4 (modo Reel)")
    p.add_argument("--portada", help="URL pública de la portada jpg (opcional)")
